"""IBM Watsonx provider adapter using ibm-watson IAM authentication and REST API."""
...
class WatsonProvider(BaseProvider):
    ...
    def __init__(
        ...
        watson_foundation_model_id: str = "meta-llama/llama-3-8b-instruct",
    ) -> None:
        ...
        self._watson_iam_authenticator: IAMAuthenticator | None = None
    ...
    def _get_watson_iam_authenticator(self) -> IAMAuthenticator:
        """Lazily initialize the IBM Cloud IAM authenticator."""
        if self._watson_iam_authenticator is None:
            self._watson_iam_authenticator = IAMAuthenticator(apikey=self._watson_api_key)
        return self._watson_iam_authenticator

    def _fetch_watson_iam_access_token(self) -> str:
        """Retrieve a fresh IBM Cloud IAM bearer token via the ibm-watson SDK stack."""
        watson_iam_access_token = self._get_watson_iam_authenticator().token_manager.get_token()
    ...
    async def execute_comparison_shopping_query(
        ...
        if not self._watson_api_key.strip():
            raise ValueError("WATSON_API_KEY is not configured.")
        if not self._watson_project_id.strip():
            raise ValueError("WATSON_PROJECT_ID is not configured.")
